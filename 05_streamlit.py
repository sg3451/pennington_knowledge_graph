...
# ---------------------------------------------------------------------------
# LLM free-text query — with code fence stripping
# ---------------------------------------------------------------------------

GRAPH_SCHEMA = """
Nodes:
  Paper       (paper_id, title, publication_year, cited_by_count, abstract, doi, pmid, is_oa, has_full_text)
  ...
  BioEntity   (normalized_id, entity_type ['gene','disease','chemical','species','cell_line','biomedical'], display_name)
...
Relationships:
  ...
  (Paper)-[:MENTIONS]->(BioEntity)
  (Paper)-[:FUNDED_BY]->(Grant)
  (Paper)-[:PUBLISHED_IN]->(Journal)

CRITICAL RULES:
  - Paper nodes have NO journal_name property — always use OPTIONAL MATCH for journals
  - Use toLower() for all text matching
...
def nl_to_cypher(question: str) -> tuple[str, str]:
    ...
    if not ANTHROPIC_API_KEY:
        return "", "No ANTHROPIC_API_KEY set in .env"
...
def summarize(question: str, results: list[dict]) -> str:
    ...
    prompt = (
        ...
        f"Data retrieved from the Pennington Biomedical knowledge graph:\n"
        f"{json.dumps(results[:15], indent=2)}\n\n"
        "Write 2-3 sentences summarizing the key findings. "
        "Be specific with numbers. Do not mention Cypher or databases."
    )
